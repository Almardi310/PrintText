...
function addTextToEndOfArray() {
    arrayOfTexts.push(getText());
}



function createNewRowInTopOfTable() {
    textID = "text" + (arrayOfTexts.length - 1) ;
    buttonID = "button" + (arrayOfTexts.length - 1) ;
    newRow = document.getElementById("myTable").insertRow(0);
    var id = arrayOfTexts.length - 1;
    newRow.innerHTML = `
        <tr id="` + id + `">
            <td id="` + textID + `" class="editText" ondblclick="editText(this);">` + arrayOfTexts[arrayOfTexts.length - 1] + `</td>
            <td id="` + buttonID + `" class="deleteRow" onclick="deleteRow(this);">
                <svg xmlns="http://www.w3.org/2000/svg" height="1em" style="fill: red;" viewBox="0 0 448 512">
